...
window.onload = function () {
    ...
    document.getElementById('pm_evaluation_form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const result = await axios.post('/pm_evaluation', { content1, content2, score1, score2, project_no, non_evaluator_no });
            console.log(result);
...
async function setProjectSelector() {
    // project List 받아오기
    const project_list = await axios.get('/pm_evaluation/project_list');
...
async function getParticipatedEmployee(project_no) {
    // 직원 리스트 받아오기
    const employee_list = await axios.get(`/pm_evaluation/employee_list/${project_no}`);
